...
`;
        ...
        btnEliminarFila.addEventListener("click", () => {
            FunciionEliminarCamper(camper.id, camper.nombre);
        });
...
async function guardarCamper(){
    try {
        ...
        if(edad < 18){
            mostrarMensaje("La edad minima es 18 años ", "error");
            return
        };
    ...
    lineas.forEach((linea, indice)=>{
        ...
        if(linea){
            ...
            if(partes.length === 2){
                ...
                if(nombre && !isNaN(puntaje) && puntaje >= 0 && puntaje <= 100){
                    modulos.push({
                        id: indice + 1,
                        nombre,
                        puntaje
...
async function crearCamper(camper){
    try {
        ...
        mostrarMensaje("Error al crear: ", + error.mensaje, "error")
    }
}

async function FunciionEliminarCamper(id, nombre){
    try {
        ...
        mostrarMensaje(`Camper ${nombre} ha sido eliminado`, "exito");
        cargarCampers();
    } catch (error) {
